...
# 終了届をOUT出力
pyautogui.click(x=1254, y=307)  # 月次処理を選択
pyautogui.click(x=1122, y=373)  # 製造終了届を選択
time.sleep(0.5)
pyautogui.write(last_month.strftime('%Y%m'), interval=0.1)   # 年月を入力
pyautogui.press("enter")
pyautogui.write(tenth.strftime("%Y%m%d"), interval=0.1) # 年月日を入力
pyautogui.press("enter", presses=3, interval=0.3)       # 終了届を出力
time.sleep(1)
pyautogui.press("F12")

# 法定台帳を法定台帳フォルダにpdf保存
pyautogui.click(x=1085, y=410)  # 法定台帳を選択
time.sleep(0.5)
pyautogui.write(last_month.strftime('%Y%m'), interval=0.1)      # 年月を入力
pyautogui.press("enter")
