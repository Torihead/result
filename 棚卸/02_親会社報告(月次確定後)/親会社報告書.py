...
def main():
    ...
    # Sheet 3つの表を繰り返し処理
    for i in ws:
        rowcell_B = i.Range("B4").End(-4121).Row        # 合計行の1つ上の行を取得
        sum_row = rowcell_B + 1                         # 合計行

        i.Range(f"B{sum_row}").Value = f"=SUM(B4:B{rowcell_B})"             # B列にSUM関数
        i.Range(f"B{sum_row}").NumberFormat = "#,##0"
    ...
    paste_row = ws[0].Range("A4").End(-4121).Row + 3                    # 再度、最終行を設定（取得）する。

    ws[0].Range(f"A{paste_row}:H{paste_row}").PasteSpecial(Paste=-4163) # A最終行の3つ下に貼り付け
    ws[0].Range(f"A{paste_row}:H{paste_row}").NumberFormat = "#,##0"
    excel.CutCopyMode = False
    ...
    for name, sheet_name in filename_map.items():       # 辞書用メソッドitems()で、name:sheet_nameの要素を扱う
        ...
        dest_wb.Activate()
        time.sleep(2)
        src_sheet.Copy(Before=dest_wb.Sheets(dest_wb.Sheets.Count)) # After=では、別ブックにコピーできず
        time.sleep(2)
